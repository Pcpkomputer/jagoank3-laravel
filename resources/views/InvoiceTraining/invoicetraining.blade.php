...
<div class="container-fluid" style="margin-bottom:35px">
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Invoice Training</h1>
      </div>
      <div>
          <!-- <a href="/admin/invoicetraining/create"><button class="btn btn-success mb-3">Tambah</button></a> -->
      </div>
  ...
  <tbody>
     @foreach($invoicetraining as $value)
      <tr>
        <td>{{$value->id_invoicetraining}}</td>
        <td>{{$value->kode_invoice}}</td>
        <td>{{$value->namapaketpelatihan}}</td>
        <td>{{$value->nama}}</td>
        <td>{{$value->tanggalpemesanan}}</td>
        <td>{{$value->belisaatpromo}}</td>
        <td>{{$value->status}}</td>
        <td>
                <a href="/admin/invoicetraining/detail/{{$value->id_invoicetraining}}"><button class="btn btn-primary mb-3">Detail</button></a>
                <a href="/admin/invoicetraining/delete/{{$value->id_invoicetraining}}"><button class="btn btn-danger mb-3">Delete</button></a>
          </td>
      <tr>
     @endforeach
  </tbody>
